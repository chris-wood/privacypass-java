          ...
          size_t msg_len)
{
    ...
    if (BN_mod_mul(z, blind_z, secret, n, bn_ctx) != ERR_LIB_NONE) {
        BN_free(n);
        printf("BN_mod_mul bad\n");
        return -1;
    }
    BN_free(n);

    if (brsa_signature_init(sig, _rsa_size(pk->evp_pkey)) != 0) {
        printf("brsa_signature_init bad\n");
        return -1;
    }
...
{
    if (_rsa_parameters_check(pk->evp_pkey) != 0) {
        printf("_rsa_parameters_check bad\n");
        return -1;
    }
    const size_t modulus_bytes = _rsa_size(pk->evp_pkey);
    if (blind_sig->blind_sig_len != modulus_bytes || secret->secret_len != modulus_bytes) {
        ERR_put_error(ERR_LIB_RSA, 0, RSA_R_DATA_TOO_LARGE_FOR_MODULUS, __FILE__, __LINE__);
        printf("modulus length bad\n");
        return -1;
    }

    BN_CTX *bn_ctx = BN_CTX_new();
    if (bn_ctx == NULL) {
        printf("BN_CTX_new bad\n");
        return -1;
    }
    BN_CTX_start(bn_ctx);
...
int
brsa_blind_signature_import(const BRSAContext *context, BRSABlindSignature *blind_sig, const uint8_t *encoded_blind_sig,
                            const size_t encoded_blind_sig_len)
{
    (void) context;

    blind_sig->blind_sig = (uint8_t *)malloc(encoded_blind_sig_len);
    blind_sig->blind_sig_len = encoded_blind_sig_len;
    memcpy(blind_sig->blind_sig, encoded_blind_sig, encoded_blind_sig_len);
    return 0;
}
